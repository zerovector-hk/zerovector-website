export default {
  ...
  tech: {
    title: '技术与方法',
    sub: '一条可复核的分析管线：每一步都有明确的产出与留痕。',
    stages: [
      { title: '数据接入', desc: '全节点与多源链上数据采集，统一清洗、校验与标准化。', tag: '产出：链上原始数据集' },
      { title: '图谱构建', desc: '地址聚类与交易关系建模，构建可查询的资金流图谱。', tag: '产出：交易关系图谱' },
      { title: '归因分析', desc: '行为建模与跨链追踪，形成可复核的归因结论。', tag: '产出：归因分析结论' },
      { title: '证据输出', desc: '证据保全与链式保管，产出符合司法程序要求的报告。', tag: '产出：法庭级取证报告' },
    ],
